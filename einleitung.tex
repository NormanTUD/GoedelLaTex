...
unerkannt bleibt, ist mein Kummer; \so{ich freue mich}, daß es vielmehr eine 
...
\centeredquote{So if there's a statement,\\
no proof to be heard,\\
but look like it's true,\\
'member my words:\\
if you take a math system like arithmetics,\\
who's truths can be listed just like in a list,\\
you either get statements that can't be proven\\
within the own system; or you get statements,\\
that both true and false, right at the same time,\\
which means that the system gon' blow up your mind,\\
and you can only hope it's not the second,\\
cause 1's incompleteness,\\
2's inconsistent.}{Godel's Incompleteness Rap\footcite{goedelrap}}
...
wie z.\,B. \frq $45 + 16 = 1$\flq, \frq $1 + 1 = 2$\flq, aber auch Sätze der Form
\frq der Satz mit der Nummer $m$ ist nicht beweisbar\flq.

\subsubsection*{Warum muss ein selbstreferenzieller Satz existieren?}

Man könnte einwenden: nur weil alle Sätze aufgezählt werden, heißt das noch nicht,
dass es einen Satz gibt, der \textit{zufällig} über seine eigene Nummer spricht.
Vielleicht reden alle Sätze der Form \frq der Satz mit Nummer $m$ ist nicht beweisbar\flq\
immer nur über \textit{andere} Sätze? Dieser Einwand ist berechtigt -- und genau deshalb
beruht \textsc{Gödel}s Beweis \textit{nicht} auf Zufall, sondern auf einer gezielten
diagonalen Konstruktion.

Die Konstruktion funktioniert wie folgt. Wir betrachten \textit{Klassenzeichen}:
das sind Formeln mit genau einer freien Variable (vom Typ der natürlichen Zahlen).
Diese lassen sich in eine Folge ordnen\footnote{Etwa nach steigender Gliedersumme
und bei gleicher Summe lexikographisch.} und durchnummerieren:
$R(1), R(2), R(3), \dots$

Die Operation $[R(n); n]$ bedeutet: nimm das $n$-te Klassenzeichen $R(n)$ und
setze für seine freie Variable das Zahlzeichen für $n$ selbst ein. Damit entsteht
eine \textit{Satzformel} (ein Satz ohne freie Variable), die \textit{etwas über die
Zahl $n$ aussagt}.

Nun definiert \textsc{Gödel} eine Klasse $K$ so:
$$
n \in K \equiv \overline{Bew}[R(n); n]
$$
Das heißt: $n$ gehört zu $K$ genau dann, wenn der Satz $[R(n); n]$ \textit{nicht beweisbar} ist.

Der entscheidende Punkt: Die Klasse $K$ ist im System selbst definierbar (weil alle
verwendeten Begriffe -- \frq Klassenzeichen\flq, \frq beweisbar\flq, die Einsetzungsoperation --
rekursiv sind). Also gibt es nach dem Komprehensionsaxiom (Axiom IV) ein Klassenzeichen $S$,
das genau $K$ ausdrückt. Dieses $S$ kommt irgendwo in der Aufzählung vor, sagen wir
an Position $q$:
$$
S = R(q)
$$

Jetzt fragen wir: Gehört $q$ selbst zu $K$? Der Satz $[R(q); q]$ -- also $[S; q]$ --
behauptet genau das. Und \textit{dieser Satz ist} $S$ mit eingesetztem $q$. Die
Selbstreferenz ist also \textit{keine Hoffnung auf einen glücklichen Zufall in der
Nummerierung}, sondern eine \textit{erzwungene Konsequenz} der diagonalen Einsetzung:
man nimmt das $q$-te Klassenzeichen und setzt $q$ für seine eigene freie Variable ein.

Die folgende Tabelle zeigt das Schema. Jede Zeile steht für ein Klassenzeichen $R(n)$,
jede Spalte für die Einsetzung der Zahl $m$. Der Eintrag in Zeile $n$, Spalte $m$
ist der Satz $[R(n); m]$:

\begin{center}
\begin{tabular}{c|cccccc}
	& $m = 1$ & $m = 2$ & $m = 3$ & $\dots$ & $m = q$ & $\dots$ \\
	\hline
	$R(1)$ & $[R(1); 1]$ & $[R(1); 2]$ & $[R(1); 3]$ & $\dots$ & $[R(1); q]$ & $\dots$ \\
	$R(2)$ & $[R(2); 1]$ & $[R(2); 2]$ & $[R(2); 3]$ & $\dots$ & $[R(2); q]$ & $\dots$ \\
	$R(3)$ & $[R(3); 1]$ & $[R(3); 2]$ & $[R(3); 3]$ & $\dots$ & $[R(3); q]$ & $\dots$ \\
	$\vdots$ & $\vdots$ & $\vdots$ & $\vdots$ & $\ddots$ & $\vdots$ & \\
	$R(q) = S$ & $[R(q); 1]$ & $[R(q); 2]$ & $[R(q); 3]$ & $\dots$ & $\mathbf{[R(q); q]}$ & $\dots$ \\
	$\vdots$ & $\vdots$ & $\vdots$ & $\vdots$ & & $\vdots$ & $\ddots$ \\
\end{tabular}
\end{center}

Die \textbf{Diagonale} dieser Tabelle enthält genau die Sätze $[R(n); n]$ -- also
diejenigen Sätze, bei denen das $n$-te Klassenzeichen auf seine \textit{eigene}
Positionsnummer angewandt wird:

\begin{center}
\begin{tabular}{cl}
	\textbf{Diagonaleintrag} & \textbf{Bedeutung} \\
	\hline
	$[R(1); 1]$ & Was sagt das 1. Klassenzeichen über die Zahl 1? \\
	$[R(2); 2]$ & Was sagt das 2. Klassenzeichen über die Zahl 2? \\
	$[R(3); 3]$ & Was sagt das 3. Klassenzeichen über die Zahl 3? \\
	$\vdots$ & $\vdots$ \\
	$[R(q); q]$ & Was sagt das $q$-te Klassenzeichen über die Zahl $q$? \\
	$\vdots$ & $\vdots$ \\
\end{tabular}
\end{center}

Die Klasse $K$ wird nun definiert als: $n \in K$ genau dann, wenn der
Diagonaleintrag $[R(n); n]$ \textit{nicht beweisbar} ist. Da $K$ im System
definierbar ist, existiert ein Klassenzeichen $S = R(q)$ dafür. Der Satz
$[R(q); q]$ sagt also aus: \frq $q \in K$\flq, d.\,h. \frq $[R(q); q]$ ist
nicht beweisbar\flq\ -- und das \textit{ist} der Satz $[R(q); q]$ selbst.
...
\begin{tabular}{rp{10cm}}
	...
	\hline
	1 & Alle Klassenzeichen werden aufgezählt: $R(1), R(2), R(3), \dots$ \\
	2 & Die Diagonaloperation $[R(n); n]$ wird definiert (setze $n$ in das $n$-te Klassenzeichen ein). \\
	3 & Die Klasse $K$ wird definiert: $n \in K \equiv \overline{Bew}[R(n); n]$. \\
	4 & $K$ ist im System definierbar $\Longrightarrow$ es existiert ein Klassenzeichen $S$ für $K$. \\
	5 & $S$ hat eine Position in der Aufzählung: $S = R(q)$ für ein bestimmtes $q$. \\
	6 & Der Satz $[R(q); q] = [S; q]$ sagt aus: \frq $q \in K$\flq, also: \frq $[R(q); q]$ ist nicht beweisbar\flq. \\
	7 & Aber $[R(q); q]$ \textit{ist} genau dieser Satz $\Longrightarrow$ er behauptet seine eigene Unbeweisbarkeit. \\
\end{tabular}
\end{center}

\textbf{Warum muss $q$ existieren?} Nicht weil man hofft, dass irgendein Satz
\frq zufällig\flq\ über seine eigene Nummer redet. Sondern weil:
\begin{enumerate}
	\item Der Begriff \frq beweisbar\flq\ im System ausdrückbar ist (§ \refsatz{Bew}).
	\item Man daraus die Klasse $K$ definieren kann (über die Diagonale).
	\item Jede im System definierbare Klasse ein Klassenzeichen hat (Axiom IV).
	\item Jedes Klassenzeichen in der Aufzählung vorkommt, also eine Position $q$ hat.
\end{enumerate}

Die Selbstreferenz entsteht also \textit{strukturell} durch die Diagonalkonstruktion --
ähnlich wie bei Cantors Diagonalargument eine Zahl erzwungen wird, die nicht in der
Liste steht. Der Satz enthält \textit{syntaktisch} keine explizite Selbstreferenz
(er spricht nur über \frq die Formel mit der Gödelnummer $q$\flq), und erst
\textit{nachträglich} stellt sich heraus, dass diese Gödelnummer seine eigene
ist.\mnote{Siehe dazu Fußnote \ref{zufaellig}. \textsc{Gödel} selbst spricht davon,
dass sich dies \frq gewissermaßen zufällig\flq\ herausstellt -- gemeint ist damit nicht,
dass die Konstruktion auf Zufall beruht, sondern dass der Satz \textit{strukturell}
keine explizite Selbstreferenz enthält (wie etwa \frq dieser Satz ist falsch\flq).
Die Typentheorie \textsc{Russell}s kann daher diese Art der Selbstbezüglichkeit
nicht ausschließen.}
...
Das letzte Ressort könnte sein, ihn als Axiom zu setzen. Aber die Konstruktion lässt
sich sofort wiederholen: im erweiterten System gibt es wieder eine Aufzählung aller
Klassenzeichen, wieder eine Diagonale, wieder eine Klasse $K'$, wieder ein
Klassenzeichen $S'$ mit Position $q'$ -- und wieder einen unbeweisbaren Satz
$[R'(q'); q']$.\mnote{Dieser Satz könnte auch aussagen: $\thicksim \thicksim \thicksim \text{Bew}(4922) $, wenn
man mehr als die absolute Minimalmenge nimmt. Vgl. dazu Kommentar \ref{minimalmenge}. Aber das würde die
Sache nur komplizierter machen.}
Das geht ewig so weiter. Wir haben (nach unendlich viel
...
\subsubsection*{Die Gödelisierung im Detail}
...
definiert und die Eineindeutigkeit von Primfaktorzerlegungen ausnutzt.\mnote{Jede
...
Die Zuordnung der Grundzeichen zu Zahlen:

\begin{center}
\begin{tabular}{cl}
	\textbf{Zeichen} & \textbf{Zugeordnete Zahl} \\
	\hline
	$0$ & 1 \\
	$f$ (Nachfolger) & 3 \\
	$\thicksim$ (nicht) & 5 \\
...
\begin{tabular}{rp{11cm}}
	\textbf{Schritt} & \textbf{Umwandlung} \\
	\hline
	1 & Natürliche Sprache: \frq es gilt nicht, dass 4\flq \\
	2 & Formalisierung: $\thicksim 4$ \\
	3 & Peano-Darstellung: $\thicksim ffff0$ (mit der Nachfolgerfunktion $f$)\mnote{Siehe dazu § \refsatz{0 N x}} \\
	4 & Gödelisierung (siehe unten) \\
\end{tabular}
\end{center}

Für Schritt 4 nehmen wir die Position $p$ im String (angefangen von links mit 1)\mnote{Siehe dazu § \refsatz{Z(n)}}
und nehmen die $p$te Primzahl als Basis, die Ziffer für das Zeichen als Exponent:

\begin{center}
\begin{tabular}{ccccc}
	\textbf{Position $p$} & \textbf{Zeichen} & \textbf{Zeichenwert} & \textbf{$p$-te Primzahl} & \textbf{Faktor} \\
	\hline
	1 & $\thicksim$ & 5 & 2 & $2^5 = 32$ \\
	2 & $f$ & 3 & 3 & $3^3 = 27$ \\
	3 & $f$ & 3 & 5 & $5^3 = 125$ \\
	4 & $f$ & 3 & 7 & $7^3 = 343$ \\
	5 & $f$ & 3 & 11 & $11^3 = 1331$ \\
...
Das Produkt ergibt die Gödelnummer:
$$
2^5 \times 3^3 \times 5^3 \times 7^3 \times 11^3 \times 13^1 = 640\,972\,332\,000
$$

Diese Zahl lässt sich durch Primfaktorzerlegung jederzeit 1:1 in den Ursprungssatz
zurückwandeln\mnote{\customlabel{eineindeutigkeit}{\arabic{commentaryNumber}}Die Eineindeutigkeit der Zuordnung von Zahlenwerten
...
dafür argumentieren möchte, sondern eher Platonist bin).}, so dass gilt:

$$ 640\,972\,332\,000 \equiv \thicksim ffff0 \equiv \text{\frq es gilt nicht, dass 4\flq} $$

Die Zahl $640\,972\,332\,000$ ist damit eineindeutig ebendiesem Satz zugeordnet und mit dem
gewählten Alphabet keinem anderen ($\longrightarrow$ Eineindeutigkeit).

\subsubsection*{Beweisbarkeit als formaler Begriff}
...
$$
haben (wobei $n$ seine gödelisierte Satznummer ist und sich durch die oben gezeigte
diagonale Konstruktion ergibt, dass $n = m$\footnote{Wie \textsc{Gödel} es selbst sagt, wäre es kein
...
siehe Fußnote \ref{leichtaufzuschreiben}.}),
dann haben wir einen Satz, der seine eigene Unbeweisbarkeit
...
Die Logik des Widerspruchs nochmals als Tabelle:

\begin{center}
\begin{tabular}{lp{9cm}}
	\textbf{Annahme} & \textbf{Konsequenz} \\
	\hline
	$[R(q); q]$ ist beweisbar & $\Longrightarrow$ $[R(q); q]$ ist wahr
		$\Longrightarrow$ $q \in K$
		$\Longrightarrow$ $[R(q); q]$ ist \textit{nicht} beweisbar. \textbf{Widerspruch.} \\[0.5em]
	$\text{Neg}([R(q); q])$ ist beweisbar & $\Longrightarrow$ $q \notin K$
		$\Longrightarrow$ $[R(q); q]$ \textit{ist} beweisbar
		$\Longrightarrow$ $[R(q); q]$ und seine Negation sind beweisbar. \textbf{Widerspruch mit $\omega$-Widerspruchsfreiheit.} \\[0.5em]
	\hline
	\textbf{Ergebnis} & $[R(q); q]$ ist \textit{unentscheidbar}: weder beweisbar noch widerlegbar. Aber durch metamathematische Überlegung als \textit{wahr} erkennbar. \\
\end{tabular}
\end{center}
