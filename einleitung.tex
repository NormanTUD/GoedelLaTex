...
die zugewiesene Nummer 4922 hat, dann behauptet
der Satz seine eigene Unbeweisbarkeit. Diese Selbstreferenz ist keine
explizite Bezugnahme des Satzes auf sich selbst (die durch eine Typentheorie
verboten werden könnte), sondern ergibt sich als Konsequenz der
diagonalen Konstruktion: der Satz behauptet zunächst nur die Unbeweisbarkeit
einer bestimmten Formel mit einer bestimmten Gödelnummer, und es stellt sich
nachträglich heraus, dass diese Gödelnummer gerade diejenige des Satzes selbst
...
keine explizite Selbstreferenz enthält. Die Typentheorie \textsc{Russell}s kann daher
diese Art der Selbstbezüglichkeit nicht ausschließen.}
...
\begin{enumerate}
	...
	\item Nun nehmen wir die Position $p$ im String (angefangen von links mit 1)\mnote{Siehe dazu § \refsatz{Z(n)}}
		...
		$$ 640972332000 \equiv \thicksim ffff0 \equiv \text{\frq es gilt nicht, dass 4\flq} $$
...
$$
haben (wobei $n$ seine gödelisierte Satznummer ist und sich durch die diagonale
Konstruktion ergibt, dass $n = m$\footnote{Wie \textsc{Gödel} es selbst sagt, wäre es kein
