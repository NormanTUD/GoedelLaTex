...
\mnote{$\left(n\right) N x \equiv \text{\glqq}f\text{\grqq.} \left(n - 1\right) N x$, die 3 ist das \glqq $f$\grqq, vgl. § \refsatz{Konkat}}
	...
	% 18:
	\item \customlabel{Typ1'}{18}$\text{Typ}_1'\left(x\right) \equiv \left(Em, n\right) \left\{m, n\leqq x \& \left[m = 1 \lor 1\textit{ Var } m\right] \& x = n N \left[R\left(m\right)\right]\right\}$\footnote{$m, n \leqq x$ steht für: $m \leqq x \& n \leqq x$ (ebenso für mehr als 2 Variable).}
	\let\thefootnote=\originalfootnote
