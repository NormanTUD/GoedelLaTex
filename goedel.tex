...
\ifodd\value{showComments}
	...
	\newcommand{\mnote}[1]{\marginpar{\textcolor{darkgray}{\small\arabic{commentaryNumber}: #1}}\stepcounter{commentaryNumber}}
\else 
	\newcommand{\mnote}[1]{}
\fi
...
Die Entwicklung der Mathematik in der Richtung zu größerer\mnote{Das Hilbert-Programm forderte, dass alle mathematischen Sachverhalte zurückführbar sein müssen auf einfache logische Operationen. In der Principia Mathematica haben Russell und Whitehead genau das zu erreichen versucht.}
...
zeigen, daß die Begriffe \mnote{Vgl. § 32, 44, 46}\glqq Formel\grqq, 
...
\mnote{Da jede Zahl eineindeutig einer Formel zugeordnet werden kann (durch Primfaktorzerlegung),
...
\mnote{$\exists n: n \in K \wedge \thicksim Bew \left[R\left(n\right); n\right]$}
\begin{equation}
\label{formel1}
...
(wobei $Bew\ x$ bedeutet: $x$ ist eine beweisbare Formel\mnote{Vgl. § 46}).
...
\begin{enumerate}[I.]
	...
	\begin{enumerate}[1.]
		\item $\left(Eu\right)\left(v\Pi\left(u\left(v\right) \equiv a\right)\right)$\mnote{$\exists u: \forall v: u(v) \equiv a$}
	\end{enumerate}
...
$n_1, n_2, \dots n_k$ die Zahl $2^{n_1}, 3^{n_2}, \dots p_k^{nk}$\mnote{$\glqq f\left(0\right)\grqq\ = 2^1 + 3^{11} + 5^{1} + 7^{13} = 96\,889\,187\,561$, 96\,889\,187\,561 ist eineindeutig zugeordnet zu \glqq$f\left(0\right)$\grqq}
...
ist daher rekursiv.\mnote{Es folgt die Liste der Axiome und Definitionen aus diesen Axiomen zum Herleiten der Unvollständigkeitssätze (§§ 1--46).}
...
\item $\text{Prim }\left(x\right) \equiv \overline{\left(Ez\right)} \left[z \leqq x \& z \not = 1 \& z \not = x \& x/z\right] \& x > 1$\mnote{$\text{Prim }\left(x\right) \equiv \not\exists z: \left(z \leqq x \wedge z \not= 1 \wedge z \not= x \wedge x/z\right) \wedge x > 1$}
	...
	$Pr\ \left(n + 1\right) \equiv \epsilon y \left[y \leqq \left\{ Pr\ \left(n\right) \right\}! + 1 \& \text{Prim }\left(y\right) \& y > Pr\ \left(n\right)\right] $%
		\mnote{$Pr\ \left(n\right) \equiv \exists y: y \leqq \left(Pr\ \left(n\right)\right)! + 1 \wedge \text{Prim }\left(y\right) \wedge y > Pr\ \left(n\right)$, vgl. §§ 2, 3, 4, 5}
	...
	\item $n Gl x \equiv \epsilon y \left[y \leqq x \& x/\left(n Pr x\right)^y \& \overline{x/n Pr x)^{y + 1}}\right]$%
		\mnote{$n Gl x \equiv \exists y: y \leqq x \wedge x/\left(n Pr x\right)^y \wedge \thicksim \left(\left(x / n Pr x\right)^{x + 1}\right)$, vgl. §§ 1, 5}
...
\mnote{$l\left(x\right) \equiv \exists y: y \leqq x \wedge y Pr x > 0 \wedge \left(y + 1\right) Pr x = 0$}
	...
	\item $R\left(x\right) \equiv 2^x$\mnote{Die Zahl $x$ an erster Stelle bedeutet: Die erste Primzahl 2 hoch die $x$, die kodiert werden soll. $2^x$ degödelisiert immer $x$ für $x > 0$}

	$R\left(x\right)$ entspricht der nur aus der Zahl $x$ bestehenden Zahlenreihe (für $x > 0$).

	% 10:
	\item $E\left(x\right) \equiv R\left(11\right) \pmstar x \pmstar R\left(13\right)$\mnote{$E\left(x\right) \equiv \text{\glqq}(\text{\grqq .} x \text{. \glqq})\text{\grqq}$, vgl. § 8}
...
\mnote{$n\text{ Var } x \equiv \exists z: \left[\text{\glqq)\grqq} < z \leqq x \wedge \text{Prim }\left(z\right) \wedge x = z^n\right] n \not= 0$, die 13 ist die Gödelnummer für \glqq)\grqq, vgl. § 2}

	%12:
	$x$ ist eine \textit{Variable $n$-ten Typs}.

	\item $\text{Var }\left(x\right) \equiv \left(En\right)\left[n \leqq x \& n \text{ Var } x\right]$%
\mnote{$\text{Var }\left(x\right) \equiv \exists n: \left(n \leqq x \wedge \left(n \text{ Var } x\right)\right)$, vgl. § 11}

	$x$ ist eine \textit{Variable}.

\item $\text{Neg }\left(x\right) \equiv R\left(5\right) \pmstar E\left(x\right)$\mnote{$\text{Neg }\left(x\right) \equiv \thicksim x$, vgl. § 8.}
...
\mnote{$x \text{ Dis } y \equiv x\text{ . \glqq}\lor\text{\grqq . } y$, vgl. § 8.}
	...
	\item $x \text{ Gen } y \equiv R\left(x\right) \pmstar R\left(9\right) \pmstar E\left(y\right)$%
		...
		$x \text{ Gen } y \equiv \forall x: y$, vgl. § 8}
...
\mnote{$(n) N x \equiv \text{\glqq}f\text{\grqq .} \left(n - 1\right) N x$, die 3 ist das \glqq $f$\grqq, vgl. § 9}
	...
	$x$ ist \textit{Zeichen $n$-ten Typs}.\mnote{Siehe § 18 für Spezialfall $n = 1$}
...
\mnote{$v Fr\ x \equiv \exists n: n \leqq l\left(x\right) \wedge v Fr\ n, x$, vgl. §§ 7, 25}

	$v$ kommt in $x$ als \textit{freie Variable} vor.

	\item $Su\ x \Spvek{n; y} \equiv \epsilon z \left\{ z \leqq \left[Pr\left(l\left(x\right) + l\left(y\right)\right)\right]^{x + y} \& \left[\left(Eu, v\right) u, v \leqq x \& x = u \pmstar R\left(n Gl\ x\right) v \& z = u \pmstar y \pmstar v \& n = l\left(u\right) + 1\right]\right\}$
	...
	$\left(k + 1\right) St\ v, x \equiv \epsilon n \left\{n < k St\ v, x \& v Fr n, x \& \overline{\left(Ep\right)} \left[n < p < k St\ v, x \& v Fr p, x\right]\right\}$
...
%32:
	\item $x \text{ Imp } y \equiv \left[\text{Neg }\left(x\right)\right] \text{ Dis } y $\mnote{$x \text{ Imp } y \equiv \left(\thicksim x\right) \lor y$. Bedeutet: $x \supset y$}

	$x \text{ Con } y \equiv \text{ Neg } \left\{\left[\text{Neg }\left(x\right)\right]\text{ Dis }\left[\text{Neg }\left(y\right)\right]\right\}$\mnote{$x \text{ Con } y \equiv \left(x \supset y\right) \lor \thicksim y$. Bedeutet $x \lor y$}
...
\mnote{$v \text{ Ex } y \equiv \thicksim \left(\forall v: \lnot y\right)$. Bedeutet \glqq alle $y$ in $v$ sind wahr\grqq, vgl. §§ 13, 15}
...
\mnote{$A_1 - Ax\left(x\right) \equiv \exists y: y \leqq x \wedge \text{Form }\left(y\right) \wedge x = (y \lor y) \supset y$, vgl. § 14, 23, 32, 34}
% Was bringt y \lor y???
...
\mnote{$M - Ax\left(x\right) \equiv \exists n: n \leqq x \wedge x = n Th\ z_4$, vgl. § 33}
	...
	%44:
	\item $
		Bw\left(x\right) \equiv \left(n\right)\left\{0 < n \leqq l\left(x\right) \longrightarrow Ax\left(n Gl x\right) \lor 
		\left(Ep, q\right) \left[0, < p, q < n \& Fl\left(n Gl x, p Gl x, q Gl x\right)\right]\right\} 
		\& l\left(x\right) > 0
	$
	...
	\item $x B y \equiv Bw\left(x\right) \& \left[ l\left(x\right) \right] Gl x = y$%
		\mnote{$x$ ist Beweis für $y \equiv \left(x \text{ ist Beweisfigur } \wedge\allowbreak \left(\text{ die Länge von } x \text{ das n}-\text{te Glied der Zahl } x\right) = y\right)$, vgl. §§ 44, 7, 6} % TODO versteh ich nicht richtig mit l usw.
...
Wir setzen:
\mnote{17 ist die erste \textit{freie Variable}}
\begin{equation}
	p = 17 \text{ Gen } q
\end{equation}
