...
$\thicksim(a)$, $(a)\lor(b)$, $x\Pi(a)$ gehören (wobei $x$ eine
\let\originalfootnote=\thefootnote
\let\thefootnote=\fnachtzehna
beliebige Variable ist)\footnote{$x\Pi(a)$ ist also auch dann eine 
Formel, wenn $x$ in $a$ nicht oder nicht frei vorkommt. In diesem 
Fall bedeutet $x\Pi(a)$ natürlich dasselbe wie $a$.}
\let\thefootnote=\originalfootnote
\setcounter{footnote}{19}
$\left(a\right)\lor\left(b\right)$ nennen wir Disjunktion aus $a$ und $b$, 
$\thicksim\left(a\right)$ die Negation und $x\Pi(a)$ eine Generalisation von 
...
$n_1, n_2, \dots n_k$ die Zahl $2^{n_1}, 3^{n_2}, \dots p_k^{nk}$\mnote{$f(0) = 2^1 + 3^{11} + 5^{1} + 7^{13} = 96889187561$, 96889187561 ist 1:1 zugeordnet zu $f(0)$}
...
wir mit $\Phi(a)$. Sei nun irgend eine Klasse oder % TODO Phi geht hier iwie nicht
...
$a_1, a_2, \dots a_n$ gibt, daß $x_i = \Phi(a_i) (i = 1, 2, \dots n)$ und % TODO Auch hier ist Phi kaputt
$R(a_1, a_2, \dots a_n)$ gilt. Diejenigen
...
Funktionen an die Leerstellen einer der vorhergehenden, z.\,B. $\phi_k(x_1, x_2) = \phi_p[\phi_q(x_1, x_2), \phi_r(x_2)]$ ($p, q, r < k$). Nicht alle Variable der linken Seite müssen auch rechts vorkommen (ebenso im Rekursionsschma (2)).}
...
\begin{enumerate}[1.]
	...
	\item $Op(x\ y\ z) \equiv x = \text{Neg}(y) \lor x = y \text{ Dis } z \lor\allowbreak (Ev) \left[v \leqq x \& \text{Var}(x) \& x = v \text{ Gen } y\right]$

	\item $$
		FR(x) \equiv (n) \left\{ 0 < n \leqq l(x) \longrightarrow\allowbreak Elf(n Gl x) \lor (Ep, q)\allowbreak \left[0 < p, q < n \&\allowbreak Op(n Gl x, p Gl x, q Gl x)\right]\right\} \& l(x) > 0
		$$
	...
	\item $$
		v \text{ Geb } n, x \equiv \text{Var}(v) \& \text{Form}(x) \& (Ea, b, c) [a, b, c \leqq x \& x a = \pmstar (v \text{ Gen } b \pmstar c \& \text{Form}(b) \& l(a) + 1 \leqq n \leqq l(a) + l(v \text{ Gen } b)]
		$$
...
% TODO check hier ist eine Klammer nicht geschlossen!!!
	...
	\item $$
		L_2 - Ax(x) \equiv (Ev, q, p) \{v, q, p \leqq x \& \text{Var}(v) \& \text{Form}(p) \& v Fr p \& \text{Form}(x) \& x = [v \text{ Gen } (p\text{ Dis } q)]\}
		$$
	...
	\item $$
		Bw(x) \equiv (n)\{0 < n \leqq l(x) \longrightarrow Ax(n Gl x) \lor (Ep, q) [0, < p, q < n \& Fl(n Gl x, p Gl x, q Gl x)]\} \& l(x) > 0
		$$
...
$(Ex_0)C$ für die Funktion $\Psi_4$ gibt es ein Individuum $a$ 
...
und gegen die Operation $\Psi_1(x)$ abgeschlossen ist. 
