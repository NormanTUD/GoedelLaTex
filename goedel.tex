...
Ein Algorithmus ist eine endliche Abfolge von genau spezifizierten Schritten, um ein
mathematisches oder logisches Problem zu lösen. Die Frage, vor der \so{Hilbert} stand,
lautete: wenn man die Mathematik formalisiert, sind dann alle wahren Aussagen durch
einen Algorithmus beweisbar?

Für \so{Hilbert} schien die Sache klar, dass das so sein muss, nur fehlte im der Beweis
dafür, dass das so ist. Erst Gödel zeigte, dass das nicht geht.

Was heißt \frq beweisbar\flq? Beweise sind Zurückführungen auf einzelne Axiome mithilfe
von syntaktischen Veränderungsregeln.\mnote{Vgl. Kommentar \ref{tractatus}} 
So kann jede mathematische Aussage als Zeichenkette
...
Zeichenketten bearbeiten. Die mathematische Formel wird dabei nur sekundär inhaltlich\mnote{Die inhaltliche Betrachtung findet erst
beim Menschen statt, der versucht, das, was die Formel aussagt, zu verstehen. Der Formel selbst
liegt diese inhhaltliche Deutung nicht bei, was unter Anderem zur extrem breiten Verwendbarkeit
...
Kette an UND, XOR und NICHT-Operationen.

Gödel zeigte, dass möglich es nicht ist, alle wahren Aussagen zu beweisen, 
in dem er einen Algorithmus anbietet, der -- komplett zurückführbar auf die logischen 
Grundaussagen --, immer Sätze konstruieren kann, die nach formalen Regeln im System 
gelten, die aber nicht beweisbar sind.
...
in der Principia Mathematica solche Sätze, die sich auf sich selbst beziehen, verboten.
Aber selbst, wenn man das verbietet, lassen sich dennoch solche Sätze konstruieren.
...
definierbaren Sätze, wie z.\,B. \frq $1 + 1 = 2$\flq, aber auch \frq der Satz mit
...
der Satz seine eigene Unbeweisbarkeit, und das mehr oder weniger zufällig.\mnote{Siehe dazu Fußnote \ref{zufaellig}.}

Können wir ihn aus den logischen Regeln herleiten (d.\,h. beweisen), dann ist
er unbeweisbar. \textit{Aber wir haben ihn doch hergeleitet, d.\,h. bewiesen!}
Das Resultat: der Satz ist unentscheidbar. Wenn er wahr ist, ist er falsch,
und wenn er falsch ist, könnte er zumindest potenziell noch wahr sein.
...
nicht beweisbar\flq. Wir stehen also wieder vor dem gleichen Problem und müssten
...
Gödels Algorithmus bietet die Möglichkeit, jedem Satz eineindeutig eine solche Nummer
zuzuordnen, indem er einige Konstanten definiert, z.\,B. wird \frq 0\flq\ zu 1,
\frq f\flq\ zu 3, \frq$\thicksim$\flq\ zu 5\mnote{Siehe dazu S. \pageref{grundzeichenzahlen}.}. Außerdem nutzt er die Eineindeutigkeit von
Primfaktorzerlegungen aus. So kann man den Satz: 
...
\begin{enumerate}
	...
	\item Nun nehmen wir die Position $p$ im String (angefangen von links mit 1)\mnote{Siehe dazu § \refsatz{Z(n)}}
		...
		Wir erhalten:
		$2^5 \times 3^3 \times 5^3 \times 7^3 \times 11^3 \times 13^3 \times 17^1 = 1841513509836000$. Dies
		können wir, da es unendlich viele Primzahlen gibt, für jede beliebig lange Zeichenkette 
		bestehend aus den Grundzeichen machen. Die Zahl 1841513509836000 lässt sich durch 
		Primfaktorzerlegung prinzipiell wieder 1:1 in den Ursprungssatz zurückwandeln, so dass gilt:

		$$ 1841513509836000 \equiv \text{\frq es gilt nicht, dass 4\flq} $$
\end{enumerate}

Der Begriff der Beweisbarkeit lässt sich im System der Principia Mathematica selbst
...
darstellen: $\text{Bew}\left(x\right)$. Wenn wir also einen Satz der Form:

$$
n \vdash \thicksim \text{Bew}\left(n\right)
$$\mnote{Vgl. § \refsatz{Bew}}
(wobei $(n)$ seine gödelisierte Satznummer, d.\,h. eine lange Zahl, die eineindeutig dem
Satz zugeordnet ist, ist\footnote{Wie Gödel es selbst sagt, wäre es kein Problem,
diese Zahl für jede Aussage tatsächlich aufzuschreiben; es wäre nur sehr umständlich, siehe Fußnote \ref{leichtaufzuschreiben}.}), 
...
per definitionem unbeweisbar.

Für solche Sätze sind Systeme wie das der Principia Mathematica prinzipiell überflüssig,\mnote{Siehe wieder Fußnote \ref{leichtaufzuschreiben}}
...
Die Auswirkungen auf das Verständnis der Mathematik war gewaltig, denn plötzlich war klar,
...
Das setzt -- ähnlich wie das Halteproblem -- der Theorie der Algorithmen und dem
menschlichen Erkennen überhaupt enge Grenzen.
...
Ein System, in dem keine Existenzaussage bewiesen werden kann, die in irgendeinem
...
dass im genannten System der Begriff \frq die Formel $f$ ist beweisbar im System $S$\flq\ definierbar ist.}Dies gilt für alle ausreichend komplexen
...
(zweiter Unvollständigkeitssatz).\mnote{Vgl. Satz XI auf S. \pageref{satzxi}.}

Im Rahmen dieser Arbeit würde ich gern versuchen, den Originalalgorithmus soweit
es mir möglich ist nachzuvollziehen. Dazu habe ich die Arbeit von Gödel soweit es
mir möglich war exakt abgetippt und mit \LaTeX, einem Textsatzsystem für mathematische
Texte, komplett neu gesetzt, um einige
Dinge wie eingefärbte passende Klammern zur besseren Lesbarkeit einzuführen und
Kommentare hinzuzufügen, die die gödel'schen Formeln in eine modernere
Schreibweise übertragen. Trotz größtmöglicher Mühe un Bemühung um Genauigkeit
