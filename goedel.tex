...
\setcounter{leftRightBracketsColor}{0}
...
\mnote{$n Pr x \equiv \exists y: y  \leqq x \wedge\text{Prim } x \wedge x/y \wedge y > (n - 1) Pr\ x$. Beschreibt den $n$ten Primfaktor von $x$, vgl. \refsatz{x/y}, \refsatz{Prim}, \refsatz{0 Pr x}}
	...
	\item \customlabel{Fakultät}{4}$0! \equiv 1$\mnote{Fakultät, zweistufig-rekursive Funktionsdefinition}
		...
		$\left(n + 1\right)! \equiv \left(n + 1\right).n!$\mnote{Gleich mit $\left(n\right)! \equiv \left(n - 1\right) \times n!$}%Wirklich (n + 1)! = (n + 1)*n!???
...
\mnote{$l\left(x\right) \equiv \exists y: y \leqq x \wedge y Pr x > 0 \wedge \left(y + 1\right) Pr x = 0$}
	...
	% 8:
	\item \customlabel{Konkat}{8}$x \pmstar y \equiv \left\{ z \leqq \left[ Pr \left(l\left(x\right) + l\left(y\right)\right)\right]^{x + 1} \& 
...
\mnote{Konvertiert \glqq normale\grqq\ Zahlen zu einem Zahlzeichen. So wird die \glqq 5 \grqq\ zu $ffff1$.}
	...
	% 18:
	\item \customlabel{Typ1'}{18}$\text{Typ}_1'\left(x\right) \equiv \left(Em, n\right) \left\{m, n\leqq x \& \left[m = 1 \lor 1\textit{ Var } m\right] \& x = n N \left[R\left(m\right)\right]\right\}%
$\footnote{$m, n \leqq x$ steht für: $m \leqq x \& n \leqq x$ (ebenso für mehr als 2 Variable).}%
\mnote{$\text{Typ}_1\left(x\right) \equiv \exists m \exists n: n \leqq x \wedge m \leqq x \wedge \left(m = 1 \lor 1 Var\ m\right) \wedge x = n N\left[R\left(m\right)\right]$}
	...
	% 23:
	\item \customlabel{Form}{23}$\text{Form}\left(x\right) \equiv \left(En\right) \left\{n \leqq \left(Pr \left[l\left(x\right)^2\right]\right)^{x.\left[l\left(x\right)\right]^2} \& FR\left(n\right) \& x = \left[l\left(n\right)\right] Gl n\right\}$\footnote{Die Abschätzung $n \leqq \left(Pr \left[l\left(x\right)^2\right]\right)^{x l\left(x\right)^2}$ 
...
\mnote{$v Fr\ x \equiv \exists n: n \leqq l\left(x\right) \wedge v Fr\ n, x$, vgl. §§ \refsatz{l(x)}, \refsatz{v Fr n, x}. Macht aus dem Satz \refsatz{v Fr n, x}, der herausfindet, ob $v$ in $x$ an $n$ter Stelle frei ist die generalisierte Aussage: die Variable $v$ befindet sich überhaupt in $x$ oder nicht.}
	...
	%28:
	\item \customlabel{n St v, x}{29}$0 St\ v, x \equiv \left\{ n \leqq l\left(x\right) \& v Fr n, x \& \overline{\left(Ep\right)} \left[n < p \leqq l\left(x\right) \& v Fr p x\right]\right\}$
...
%33:
	\item \customlabel{Th}{33} $n Th\ x \equiv \epsilon y \left\{ y \leqq x^{\left(x^n\right)} \& \left(k\right) \left[k \leqq l\left(x\right) \longrightarrow \left(k Gl\ x \leqq 13 \& k Gl\ y = k Gl\ x\right) \lor \left(k Gl\ x > 13 \& k Gl\ y = k Gl\ x.\left[1 Pr\ \left(k Gl\  x\right)\right]^n\right)\right]\right\}$
