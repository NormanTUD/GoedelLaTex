...
\begin{enumerate}[I.]
	\item \begin{enumerate}[1.]
		...
		\item $x_2(0).x_1\Pi\left(x_2\left(x_1\right) \supset x_2\left(fx_1\right)\right) \supset x_1 \Pi \left(x_2\left(x_1\right)\right)$.\mnote{???}
	\end{enumerate}
...
$n_1, n_2, \dots n_k$ die Zahl $2^{n_1}, 3^{n_2}, \dots p_k^{nk}$\mnote{$f\left(0\right) = 2^1 + 3^{11} + 5^{1} + 7^{13} = 96889187561$, 96889187561 ist 1:1 zugeordnet zu $f\left(0\right)$}
...
\begin{equation*}
	\alpha\left(x\right), \beta(x, y), \gamma(x, y)
\end{equation*}
nämlich:
\begin{equation*}
	\alpha(0) = 1; \alpha\left(x\right) = 0\text{ für } x \not = 0
\end{equation*}
\begin{equation*}
...
\begin{equation*}
	R\left(\mathfrak{x}, \mathfrak{y}\right) \thicksim \left[\rho(\mathfrak{x}, \mathfrak{y}) = 0\right].
\end{equation*}
...
\begin{equation*}
	\psi\left(\mathfrak{x}, \mathfrak{y}\right) = \chi\left(\phi(\mathfrak{x}), \mathfrak{y}\right)
\end{equation*}
\begin{equation*}
...
\begin{enumerate}[1.]
	\item $x/y \equiv \left(Ez\right) [z \leqq x \& x = y.z]$\footnote{Das Zeichen $\equiv$ wird im Sinne von \glqq Definitionsgleichheit\grqq\ verwendet, vertritt also bei Definitionen entweder $=$ oder $\thicksim$ (im übrigen ist
	...
	\item $\text{Prim }\left(x\right) \equiv \overline{\left(Ez\right)} [z \leqq x \& z \not = 1 \& z \not = x \& x/z] \& x > 1$
	...
	\item $x \pmstar y \equiv \{ z \leqq [ Pr (l\left(x\right) + l\left(y\right))]^{x + 1} \& 
	(n) [n \leqq l\left(x\right) \longrightarrow (n Gl z = n Gl x] \&
	(n) [0 < n \leqq (l(y) \longrightarrow (n + l\left(x\right)) Gl z = n Gl y]\}$
	...
	% 18
	\item $\text{Typ}_1'\left(x\right) \equiv \left(Em, n\right) \left\{m, n\leqq x \& \left[m = 1 \lor 1\textit{ Var } m\right] \& x = n N \left[R(m)\right]\right\}$\footnote{$m, n \leqq x$ steht für: $m \leqq x \& n \leqq x$ (ebenso für mehr als 2 Variable).}
	\let\thefootnote=\originalfootnote
	\setcounter{footnote}{34}

	$x$ ist \textit{Zeichen ersten Typs}.

	\item $\text{Typ}_n\left(x\right) \equiv [n = 1 \& \text{Typ}_1'\left(x\right)] \lor [n > 1 \& (Ev) \{v \leqq x \& n\text{ Var } v \& x = R(v)\}]$

	$x$ ist \textit{Zeichen $n$-ten Typs}.

	\item $Elf\left(x\right) \equiv (E y, z, n) [y, z, n \leqq x \& \text{Typ}_n(y) \& \text{Typ}_{n + 1}(z) \& x = z \pmstar E(y)]$

	$x$ ist \textit{Elementarformel}.

	\item $Op(x\ y\ z) \equiv x = \text{Neg}(y) \lor x = y \text{ Dis } z \lor\allowbreak (Ev) \left[v \leqq x \& \text{Var}\left(x\right) \& x = v \text{ Gen } y\right]$

	\item $$
		FR\left(x\right) \equiv (n) \left\{ 0 < n \leqq l\left(x\right) \longrightarrow\allowbreak Elf(n Gl x) \lor (Ep, q)\allowbreak \left[0 < p, q < n \&\allowbreak Op(n Gl x, p Gl x, q Gl x)\right]\right\} \& l\left(x\right) > 0
		$$
	...
	\item $\text{Form}\left(x\right) \equiv \left(En\right) \left\{n \leqq \left(Pr \left[l\left(x\right)^2\right]\right)^{x.\left[l\left(x\right)\right]^2} \& FR\left(n\right) \& x = \left[l\left(n\right)\right] Gl n\right\}$\footnote{Die Abschätzung $n \leqq (Pr \left[l\left(x\right)^2\right])^{x l\left(x\right)^2}$ erkennt man so:Die Länge der kürzesten zu $x$ gehörigen Formelreihe kann höchstens gleich der Anzahl der Teilformeln von $x$ sein. Es gibt aber höchstens $l\left(x\right)$ Teilformeln der Länge 1, höchstens $l\left(x\right) - 1$ der Länge 2 usw., im Ganzen als höchstens $\frac{l\left(x\right)[l\left(x\right) + 1]}{2} \leqq l\left(x\right)^2$. Die Primzahlen aus $n$ können also sämtlich kleiner als $Pr\{\left[l\left(x\right)\right]^x\}$ angenommen werden, ihre Anzahl $\leqq l\left(x\right)^2$ und ihre Exponenten (welche Teilformeln von $x$ sind) $\leqq x$.}

	$x$ ist \textit{Formel} (d.\,h. letztes Glied einer \textit{Formelreihe $n$}).

	\item $$
		v \text{ Geb } n, x \equiv \text{Var}(v) \& \text{Form}\left(x\right) \& (Ea, b, c) [a, b, c \leqq x \& x a = \pmstar (v \text{ Gen } b \pmstar c \& \text{Form}(b) \& l(a) + 1 \leqq n \leqq l(a) + l(v \text{ Gen } b)]
		$$

	Die \textit{Variable} $v$ ist in $x$ an $n$-ter Stelle \textit{gebunden}.

	\item $v Fr n, x \equiv \text{Var}(v) \& \text{Form}\left(x\right) \& v = n Gl x \& n \leqq l\left(x\right) \& v \text{ Geb } n, x$

	Die \textit{Variable} $v$ ist in $x$ an $n$-ter Stelle \textit{frei}.

	\item $v Fr x \equiv (En) [n \leqq l\left(x\right) \& v Fr n, x]$

	$v$ kommt in $x$ als \textit{freie Variable} vor.

	\item $S u x \Spvek{n; y} \equiv \epsilon z \{ z \leqq [Pr(l\left(x\right) + l(y))]^{x + y} \& [(Eu, v) u, v \leqq x \& x = u \pmstar R(n Gl x) v \& z = u \pmstar y \pmstar v \& n = l(u) + 1]\}$
	...
	\item $0 St v, x \equiv \{ n \leqq l\left(x\right) \& v Fr n, x \& \overline{(Ep)} [n < p \leqq l\left(x\right) \& v Fr p x]\}$
	...
	\item $x \text{ Imp } y \equiv [\text{Neg }\left(x\right)] \text{ Dis } y $

	$x \text{ Con } y \equiv \text{ Neg } \{[\text{Neg }\left(x\right)]\text{ Dis }[\text{Neg }(y)]\}$
...
% TODO check hier ist eine Klammer nicht geschlossen!!!
	...
	\item $n Th x \equiv \epsilon y \{ y \leqq x^{(x^n)} \& (k) [k \leqq l\left(x\right) \longrightarrow (k Gl x \leqq 13 \& k Gl y = k Gl x) \lor (k Gl x > 13 \& k Gl y = k Gl x.[1 Pr (k Gl x)]^n)]\}$
	...
	\item $Z - Ax\left(x\right) \equiv (x = z_1 \lor x = z_2 \lor x = z_3)$

	\item $A_1 - Ax\left(x\right) \equiv \left(Ey\right) [y \leqq x\& \text{Form }(y) \& x = (y \text{ Dis } y) \text{ Imp } y]$
	...
	\item $L_1 - Ax\left(x\right) \equiv (Ev, y, z, n) \{v, y, z, n \leqq x \& n \text{ Var } v \& \text{Typ}_n(z) \& \text{Form}(y) \& Q(z, y, v) \& x = (v \text{ Gen } y) \text{ Imp } [Sb \left(y \substack{v\\z}\right)]\}$

	$x$ ist eine aus dem Axiomenschema III, 1 durch Einsetzung entstehende \textit{Formel}.

	\item $$
		L_2 - Ax\left(x\right) \equiv (Ev, q, p) \{v, q, p \leqq x \& \text{Var}(v) \& \text{Form}(p) \& v Fr p \& \text{Form}\left(x\right) \& x = [v \text{ Gen } (p\text{ Dis } q)]\}
		$$
	$x$ ist eine aus dem Axiomenschema III, 2 durch Einsetzung entstehende \textit{Formel}.

	\item $R - Ax\left(x\right) \equiv (Eu, v, y, n) [u, v, y, n \leqq x \& n \text{ Var } v \& (n + 1) \text{ Var } u \&  u Fr y \& \text{Form }(y) \& x = u\text{ Ex } \{v \text{Gen } [[R(u) \pmstar E(R(v))] \text{ Aeq } y ]\} ]$
	...
	\item $M - Ax\left(x\right) \equiv (En) [n \leqq x \& x = n Th z_4]$
	...
	\item $$
		Bw\left(x\right) \equiv (n)\{0 < n \leqq l\left(x\right) \longrightarrow Ax(n Gl x) \lor (Ep, q) [0, < p, q < n \& Fl(n Gl x, p Gl x, q Gl x)]\} \& l\left(x\right) > 0
		$$
	...
	\item $x B y \equiv Bw\left(x\right) \& [ l\left(x\right) ] Gl x = y$
...
$a$ gibt, so daß:
\begin{equation*}
	(n)\left[Sb\left(a\substack{v\\ Z(n)}\right) \epsilon \text{ Flg } \left(x\right)\right] \& \left[\text{Neg }(v\text{ Gen } a)\right] \epsilon \text{ Flg }(\chi)
\end{equation*}
...
\begin{equation}
	Bw_x\left(x\right) \equiv (n) [ n \leqq l\left(x\right) \longrightarrow Ax(n Gl x) \lor n Gl x) \epsilon \chi \lor
\end{equation}
\begin{equation*}
	(Ep, q) \{0 < p, q < n \& Fl(n Gl x, p Gl x, q Gl x)\}] \& l\left(x\right) > 0
\end{equation*}
(vgl. den analogen Begriff 44)

\begin{equation}
	x B_x y \equiv Bw_x\left(x\right) \& [l\left(x\right)] Gl x = y
\end{equation}
\begin{equation}
	\tag{6.1}
...
\begin{equation}
	\left(x\right) [\text{Bew}_x\left(x\right) \thicksim x \epsilon \text{Flg}(\chi)]
\end{equation}
\begin{equation}
	\left(x\right) [\text{Bew}\left(x\right) \longrightarrow \text{Bew}_x\left(x\right)]
\end{equation}
...
\begin{equation}
	\tag{8.1}
	Q\left(x, y\right) \equiv \overline{x B\chi\left\lfloor Sb\left(y\substack{19\\ Z(y)}\right)\right\rfloor}.
\end{equation}

Da $x B_x y$ [nach (6), (5)] und $Sb\left(y\substack{19\\ Z(y)}\right)$ (nach Def. 17, 31) rekursiv sind, so auch $Q\left(xy\right)$. Nach Satz V und (8) gibt es also ein \textit{Relationszeichen} $q$ (mit den \textit{freien Variablen} 17, 19), so daß gilt:

\begin{equation}
	\overline{x B_\chi\left[Sb\left(y\substack{19\\ Z(y)}\right)\right]} \longrightarrow \text{Bew}_{\chi} \left[Sb \left(q \substack{17\\ Z\left(x\right)}\substack{19\\ Z(y)}\right)\right]
\end{equation}

\begin{equation}
	x B_\chi \left[Sb\left(y\substack{19\\ Z(y)}\right)\right] \longrightarrow \text{Bew}_\chi \left[\text{Neg }Sb\left(q\substack{17\\ Z\left(x\right)}\substack{19\\ Z(y)}\right)\right]
\end{equation}
...
\begin{equation}
	Sb\left(q\substack{17\\ Z\left(x\right)}\substack{19\\ Z(p)}\right) = Sb\left(r\substack{17\\ Z\left(x\right)}\right)
\end{equation}

[nach (12)]. Setzt man nun in (9) und (10) $p$ für $y$ ein, so entsteht unter Berücksichtigung von (13) und (14):

\begin{equation}
	\overline{x B_\chi (17\text{ Gen } r)} \longrightarrow \text{Bew}_\chi \left[Sb\left(r\substack{17\\ Z\left(x\right)}\right)\right]
\end{equation}

\begin{equation}
	x B_\chi (17\text{ Gen } r) \longrightarrow \text{Bew}_\chi\left[\text{Neg } Sb\left(r\substack{17\\ Z\left(x\right)}\right)\right]
\end{equation}

Daraus ergibt sich:

\begin{enumerate}
	\item $17 \text{ Gen } r$ ist nicht $\chi$-beweisbar\footnote{$x$ ist $\chi$-beweisbar, soll bedeuten: $x \epsilon \text{Flg}\left(x\right)$, was nach (7) dasselbe besagt wie: $\text{Bew}_\chi\left(x\right)$.}. Denn wäre dies der Fall, so gäbe es (nach 6.1) ein $n$, so daß $n B_\chi (17 \text{ Gen } r)$. Nach 17 gälte also: $\text{Bew}_\chi \left[\text{Neg }Sb\left(r\substack{17\\ Z(n)}\right)\right]$, während andererseits aus der $\chi$-\textit{Beweisbarkeit} von $17 \text{ Gen } r$ auch die von $Sb \left(r\substack{17\\ Z(n)}\right)$ folgt. $\chi$ wäre also widerspruchsvoll (umso mehr $\omega$-widerspruchsvoll).
...
$\chi'$ ist aber nicht $\omega$-widerspruchsfrei, denn wegen $\overline{\text{Bew}_\chi}(17\text{ Gen } r)$ und (15) gilt: $\left(x\right) \text{Bew}_\chi Sb\left(r\substack{17\\ Z\left(x\right)}\right)$, umso mehr also:
$\left(x\right)\text{Bew}_{\chi'} Sb\left(r\substack{17\\ Z\left(x\right)}\right)$ und anderseits gilt natürlich: $\text{Bew}_{\chi'}[\text{Neg }(17\text{ Gen }r)]$\footnote{Die Existenz widerspruchsfreie und nicht $\omega$-widerspruchsfreier $\chi$ ist damit natürlich nur unter der Voraussetzung bewiesen, daß es überhaupt widerspruchsfreie $\chi$ gibt (d.\,h. daß $P$ widerspruchsfrei ist).}.
...
$$
x_0 = \phi(x_1 \dots x_n) \thicksim \left(Ef\right)\left\{ f_0 = \psi(x_2 \dots x_n) \& (k) \left[k < x_1 \longrightarrow f_{k + 1} = \mu(k, f_k, x_2 \dots x_n)\right] \& x_0 = f_{x_1}\right\}
$$
...
\begin{equation}
x_0 = \phi\left(x_1 \dots x_n\right) \thicksim \left(Ef\right) \left\{S(f_0, x_2 \dots x_n) \& (k)\left[k < x_1 \longrightarrow T(f_{k + 1}, k, f_k, x_2 \dots x_n)\right] \& x_0 = f_{x_1}\right\}
\end{equation}
...
enthält, soll eine Formel im weiteren Sinne (i.~w.~S.) heißen\footnote{Variable dritter Art dürfen dabei an allen Leerstellen für Individuenvariable stehen, z.\,B.: $y = \phi\left(x\right), F\left(x, \phi\left(y\right)\right), G\left[\psi\left(x, \phi(y)\right), x)\right]$ usw.}.
...
\end{equation}
\begin{equation*}
\left(x, x_2 \dots x_m\right) \left\{\Phi_k[\Phi_1\left(x\right), x_2 \dots x_m] = \Phi_q \left[x, \Phi_k(x, x_2 \dots x_m), x_2 \dots x_m\right]\right\}
\end{equation*}
\begin{equation*}
p, q < k
\end{equation*}
...
\begin{equation}
\left(x\right) \overline{\Phi_1\left(x\right) = 0} \& (x y) \left[\Phi_1\left(x\right) = \Phi_1(y) \longrightarrow x = y\right]
\end{equation}
...
\so{Satz XI: Sei $\chi$ eine beliebige rekursive widerspruchsfreie\footnote{$\chi$ ist widerspruchsfrei (abgekürzt als $\text{Wid }\left(x\right)$) wird folgendermaßen definiert: $\text{Wid}\left(x\right)\equiv\left(Ex\right)[\text{Form}\left(x\right)\&\text{Bew}_\chi\left(x\right)]$.}
...
\begin{equation}
\text{Wid }\left(x\right) \longrightarrow \overline{\text{Bew}_x} (17\text{ Gen } r)
\end{equation}
d.\,h. nach (6.1):

\begin{equation*}
\text{Wid }\left(x\right) \longrightarrow \left(x\right)\overline{x B_x (17\text{ Gen } r)}
\end{equation*}
Nach (13) ist $17\text{ Gen } r = Sb\left(p\substack{19\\ Z(p)}\right)$ und daher:

\begin{equation*}
\text{Wid}\left(x\right) \longrightarrow\left(x\right)\overline{x B_x Sb\left(p\substack{19\\ Z(p)}\right)}
\end{equation*}
d.\,h. nach (8.1):
