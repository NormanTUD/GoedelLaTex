...
\usepackage[a3paper,
    inner=20mm,
    outer=171mm,% = marginparsep + marginparwidth + 5mm (between marginpar and page border)
    top=20mm,
    bottom=25mm,
    marginparsep=5mm,
    marginparwidth=169mm,
...
\item $x \text{ Imp } y \equiv \left[\text{Neg }\left(x\right)\right] \text{ Dis } y $\mnote{$x \supset y \equiv \left(\lnot x\right) \lor y$}
	...
	\item $x B y \equiv Bw\left(x\right) \& \left[ l\left(x\right) \right] Gl x = y$%
		\mnote{$x$ ist Beweis für $y \equiv \left(x \text{ ist Beweisfigur } \wedge\allowbreak \left(\text{ die Länge von } x \text{ das n}-\text{te Glied der Zahl } x\right) = y\right)$, vgl. §§ 44, 7, 6}
